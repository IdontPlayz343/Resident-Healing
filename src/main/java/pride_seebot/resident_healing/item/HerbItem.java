...
public class HerbItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if ("green".equals(this.color)) {
            tooltip.add(Text.translatable("tooltip.resident_healing.heal_amount", (int) Config.greenHerbHeal/2).formatted(Formatting.GREEN));
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if ("green".equals(this.color)) {
            if (user.getHealth() < user.getMaxHealth()) {
                user.heal(Config.greenHerbHeal);
